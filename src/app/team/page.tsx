...
>>>>>>> upstream/main
  ...
  const statsData: StatsItem[] = [
    {
      id: 1,
      value: loading ? "..." : (dashboard.totalTeam ?? 0).toString(),
      label: "Total Team",
...
>>>>>>> upstream/main
    ...
    },
    {
      ...
      value: loading
        ? "..."
        : `$${dashboard.selfBusiness?.toLocaleString() ?? "0"}`,
      label: "Self Business",
...
>>>>>>> upstream/main
          {statsData.map((item) => (
            <div
              key={item.id}
              className="bg-gradient-to-br from-white/20 to-white/10 backdrop-blur-lg rounded-2xl lg:p-6 p-3 border border-white/30 shadow-2xl hover:scale-105 transition-all duration-300"
            >
              <div className="flex flex-col">
                <div className="flex items-center justify-between">
                  <div className={`font-bold text-white mb-1 ${item.id === 4 ? "lg:text-3xl text-xl" : "lg:text-2xl text-lg"}`}>
                    {item.value}
                  </div>
...
>>>>>>> upstream/main
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {staticTeamLevels.map((levelData) => (
            <div
              key={levelData.id}
              className="lg:p-6 p-3 rounded-2xl relative overflow-hidden bg-gray-900 border border-gray-800 shadow-2xl hover:scale-105 transition-all duration-500"
            >
              ...
              <div className="absolute top-0 right-2 z-20">
                <div className={`flex justify-center items-center text-lg font-bold text-white w-12 h-12 rounded-full shadow-2xl ${levelData.locked ? "bg-gradient-to-r from-gray-600 to-gray-700" : "bg-gradient-to-r from-blue-500 to-purple-500"}`}>
              ...
              <button
                ...
                className="w-full lg:py-3 py-1.5 text-sm lg:text-lg rounded-xl font-bold text-white bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-400 hover:to-purple-400 shadow-2xl transform hover:scale-105 transition-all duration-300 z-20"
              >
                View Details
              </button>
            </div>
